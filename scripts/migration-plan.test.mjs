...
test("the auth schema ships outside the globbed directory", () => {
  ...
  // App migrations (0002_*.sql, 0003_*.sql …) are allowed top-level — the
  // auth schema is the one file that must stay out until sign-in turns on.
  const pending = pendingMigrations(readdirSync(migrationsDir), []);
  assert.deepEqual(
    pending.filter((m) => m.name === AUTH_MIGRATION || m.name.startsWith("auth")),
    [],
  );
